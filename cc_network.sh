...
function networkUp () {
  CHANNEL_NAME=$CHANNEL_NAME TIMEOUT=$CLI_TIMEOUT docker-compose -f $COMPOSE_FILE -f $COUCHDB_COMPOSE_FILE up -d 2>&1
  if [ $? -ne 0 ]; then
...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.walmartlabs.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd $CURRENT_DIR
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/org2.walmartlabs.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd $CURRENT_DIR
...
# CouchDB as datastore 
COUCHDB_COMPOSE_FILE=docker-compose-couch.yaml
